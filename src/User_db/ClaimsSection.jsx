...
import styles from './ClaimsSection.module.css'; // Import the CSS module
...
const ClaimsSection = () => {
    ...
    return (
        <div className={styles.container}> {/* Use CSS module class */}
            <h2 className={styles.header}>Your Claims</h2> {/* Update header class */}
            {error && <p style={{ color: 'red' }}>{error}</p>}
            {claims.length > 0 ? (
                <table className={styles.claimsTable}> {/* Use CSS module class */}
                    <thead>
                        <tr>
                            <th>Policy Holder</th>
                            <th>Policy Type</th>
